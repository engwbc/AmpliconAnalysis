...
This should be:
 ...
 `/home/parasitology/Desktop/amplicon_analysis`
...
{
    "INPUTDIR": [
        "/path/to/MinKnow/EXPERIMENT_NAME/SAMPLE_FOLDER"
    ],
    "SAMPLE": ["SAMPLE1","SAMPLE2"],
    "barcodes": [["1","2","3"],["4","5"]],
    "OUTDIR": "/path/to/NB_AmpOutput",
...
2. The main settings to change are INPUTDIR, SAMPLE, barcodes, OUTDIR, and FRAGMENT_SIZE
    ...
    A single value can be set which will be used across all samples, e.g.,

    ```json
    "CHOPPER_LEN_PCT" : 50
    ```

    For multiple samples this can be set to, for example: 

    ```json
    "CHOPPER_LEN_PCT" : [60,75,95]
    ```
    ...
    ```json
    "AMPLICON_SORTER_MIN_PCT" : [50,75] - SAMPLE1 will use 50% and SAMPLE2 will use 75%.
    ```
